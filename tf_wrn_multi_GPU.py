...
class LSTMCell(rnn.RNNCell):
	...
	def __init__(self, num_units, W_h_list, batch_size):
		self._num_units = num_units
		self._W_h_list = W_h_list
	...
	def __call__(self, inputs, state, scope=None):
		...
		hidden_list = [None for i in range(self._batch_size)]
		for utt in range(self._batch_size):
			hidden_list[utt] = tf.matmul(h[utt,:][None, :], self._W_h_list[utt])
		hiddens = tf.concat(hidden_list, axis=0)
...
def lstm(x, sequence_length, units, scope, reverse, keep_prob, initializer):
	with tf.name_scope(scope.split('/')[-1], values=[x]):
		...
		W_h_list = [None for i in range(sequence_length.shape[0])]
		for utt in range(sequence_length.shape[0]):
			component_list = [None for i in range(4)]
			for component in range(4):
				component_list[component] = tf.nn.dropout(W_h[:,component*units:(component+1)*units], keep_prob=keep_prob, noise_shape=[units, 1])
			W_h_list[utt] = tf.concat(component_list, axis=1)

		cell = rnn.CompiledWrapper(LSTMCell(units, W_h_list, sequence_length.shape[0]))
		h, _ = tf.nn.dynamic_rnn(cell, x, time_major=True, dtype=x.dtype, sequence_length=sequence_length, swap_memory=True)
